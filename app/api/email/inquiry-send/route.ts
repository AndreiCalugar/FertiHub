...
export async function POST(request: NextRequest) {
  try {
    ...
    if (suppliersError || !inquirySuppliers || inquirySuppliers.length === 0) {
      ...
      return NextResponse.json(
        { error: "No suppliers found for this inquiry" },
        { status: 404 }
      );
    }
    ...
    for (const inquirySupplier of inquirySuppliers) {
      ...
      const emailData = {
        ...
        organizationName:
          (inquiry.user_profile as any)?.organization_name || "IVF Lab",
      ...
      if (emailResult.success) {
        ...
        console.error(
          `❌ Failed to send to ${supplier.name}:`,
          emailResult.error
        );
    ...
    console.error("❌ Error in inquiry-send API:", error);
    return NextResponse.json(
